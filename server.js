...
db.sync() // Esto solo en desarrollo, en produccion esto sería un desastre,lo borra todo, la lias parda. db.sync({force : true})
    .then(() => {
        console.log("[+] Bas de datos sincronizada")
    })
    .catch((err) => {
        console.log("[!] Error : ", err)
    })
